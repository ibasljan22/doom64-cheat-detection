...
static void M_CheatInfAmmo(player_t* player, char dat[4]);
static void M_CheatSpeedHack(player_t* player, char dat[4]);
...
cheatinfo_t cheat[] = {
	...
	{   "exi",		M_CheatInfAmmo,     0   },
	{   "exs",		M_CheatSpeedHack,   0   },
	{   NULL,       NULL,               0   }
};
...
static void M_CheatInfAmmo(player_t* player, char dat[4]) {
	...
	if (player->cheats & CF_INFAMMO) {
		player->message = "Infinite ammo ON";
	}
	else {
		player->message = "Infinite ammo OFF";
	}
}

static void M_CheatSpeedHack(player_t* player, char dat[4]) {
	player->cheats ^= CF_SPEEDHACK;
	if (player->cheats & CF_SPEEDHACK) {
		player->message = "Speed hack ON";
	}
	else {
		player->message = "Speed hack OFF";
	}
}
